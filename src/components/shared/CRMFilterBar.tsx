...
export interface FilterOption {
  ...
  /** Controlled value. Use empty string for "all". */
  value?: string;
  /** Called when the user picks an option. Empty string when "all" is chosen. */
  onChange?: (value: string) => void;
}

interface CRMFilterBarProps {
  searchPlaceholder?: string;
  /** Controlled search input. When omitted the input is uncontrolled (legacy). */
  searchValue?: string;
      ...
      <div className="flex gap-3 w-full sm:w-auto items-center">
        ...
        {actionLabel && (
          <Button
            ...
            className={`h-10 px-5 gap-2 font-medium rounded-xl shadow-none ${actionClassName || ''}`}
          >
            {actionIcon}
            {actionLabel}
          </Button>
