...
export default function PickupCRMPage() {
  ...
  const stats = [
    { label: t('statTodayTotal'), value: pickups?.length || 0 },
    {
      label: t('statActive'),
      value:
        pickups?.filter((p) =>
          ['Queued', 'Ready', 'Pending'].includes(p.status),
        ).length || 0,
      colorClass: 'text-blue-600 font-bold',
    },
    {
      label: t('statCompleted'),
      value: pickups?.filter((p) => p.status === 'Completed').length || 0,
      colorClass: 'text-emerald-500 font-bold',
    },
    {
      label: t('statCarpool'),
      value: pickups?.filter((p) => p.isCarpool).length || 0,
      colorClass: 'text-purple-600 font-bold',
    },
    {
      label: t('statNoShow'),
      value: pickups?.filter((p) => p.status === 'No Show').length || 0,
  ...
  const filters = [
    {
      ...
      options: [
        ...
        { label: t('statusQueued'), value: 'queued' },
        { label: t('statusReady'), value: 'ready' },
        { label: t('statusCompleted'), value: 'completed' },
        { label: t('statusCancelled'), value: 'cancelled' },
        { label: t('statusNoShow'), value: 'no show' },
      ],
    },
    {
      placeholder: t('filterAllTypes'),
      options: [
        { label: t('typeNormal'), value: 'normal' },
        { label: t('typeCarpool'), value: 'carpool' },
        { label: t('typeTaxi'), value: 'taxi' },
      ],
    },
  ];
    ...
    switch (status) {
      ...
      case 'Ready':
        return (
          ...
          >
            <Car className="h-3 w-3" /> {t('statusReady')}
          </Badge>
        );
      case 'Pending':
        return (
      ...
      case 'No Show':
        return (
          <Badge
            variant="outline"
            className="bg-red-100/60 text-red-700 border-none px-2 py-0.5 rounded-[12px] font-semibold text-[11px] tracking-wide gap-1"
          >
            <Ban className="h-3 w-3" /> {t('statusNoShow')}
          </Badge>
        );
      default:
  ...
  const handleUpdateStatus = (id: string, newStatus: string) => {
    queryClient.setQueryData(['pickups'], (old: Pickup[] | undefined) =>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="relative">
        ...
        <CRMTableWrapper
          title={t('todaysPickups', { count: pickups?.length || 0 })}
        >
          <Table>
            <TableHeader>
                    ...
                    <TableCell className="text-right pr-6">
                      <div className="flex flex-row justify-end items-center gap-3">
                        {pickup.status === 'Completed' ? (
                          <Button
                            onClick={() =>
                              handleUpdateStatus(pickup.id, 'Ready')
                            }
                            variant="outline"
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col gap-2.5">
                    {selectedPickup.students.map(
                      (student: string, idx: number) => (
                        <div
                          key={`${selectedPickup.id}-${student}`}
                    ...
                    {[
                      ...
                      return (
                        <Button
                          key={statusOption}
                          onClick={() =>
                            handleUpdateStatus(selectedPickup.id, statusOption)
                          }
